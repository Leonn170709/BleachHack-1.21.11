...
public class SettingBlockList extends SettingList<Block> {
	...
	@Override
	public void renderItem(MinecraftClient mc, DrawContext matrices, Block item, int x, int y, int w, int h) {
		if (item == null || item.asItem() == Items.AIR) {
			...
			matrices.drawItem(new ItemStack(item.asItem()), (int) ((x + 1) * offset), (int) ((y + 1) * offset));
