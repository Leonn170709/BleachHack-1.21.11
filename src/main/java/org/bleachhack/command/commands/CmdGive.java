...
public class CmdGive extends Command {
	...
	public void onCommand(String alias, String[] args) throws Exception {
		...
		if (args[0].equalsIgnoreCase("preset")) {
			...
			if (args[1].equalsIgnoreCase("negs")) {
				...
				tag = StringNbtReader.readCompound(
				...
				tag = StringNbtReader.readCompound("{BlockEntityTag:{Items:[{Slot:0b,id:\"minecraft:firework_rocket\",Count:1b,tag:{Fireworks:{Explosions:[{Type:0}" + s1
			...
			NbtCompound legacyStack = new NbtCompound();
			legacyStack.putString("id", Registries.ITEM.getId(item.getItem()).toString());
			legacyStack.putByte("Count", (byte) 1);

			if (args2.equalsIgnoreCase("egg")) {
				NbtCompound ct = new NbtCompound();
				ct.put("EntityTag", StringNbtReader.readCompound("{Time:1,id:\"minecraft:falling_block\",BlockState:{Name:\"minecraft:chest\"}}"));
				((NbtCompound) ct.get("EntityTag")).put("TileEntityData", tag.get("BlockEntityTag"));

				legacyStack.put("tag", ct);
			} else {
				legacyStack.put("tag", tag);
			}

			mc.player.getInventory().insertStack(legacyNbtToItemStack(legacyStack));
			return;
		}

		ItemStack item = new ItemStack(
				Registries.ITEM.get(Identifier.of("minecraft:" + args[0].toLowerCase(Locale.ENGLISH))));
		...
		if (args.length >= 4)
			try {
				...
				legacyStack.putString("id", Registries.ITEM.getId(item.getItem()).toString());
				legacyStack.putByte("Count", (byte) 1);
				legacyStack.put("tag", StringNbtReader.readCompound(args[3]));

				item.applyComponentsFrom(legacyNbtToItemStack(legacyStack).getComponents());
	...
	// 1.19.4 built modified items via the old raw-NBT ItemStack format ("tag" holding Damage,
	// Enchantments, display, BlockEntityTag, EntityTag, ...) and slammed it on with setNbt(). 1.21.11
	// replaced all of that with typed DataComponents and removed setNbt()/getNbt() entirely, so there's
	// no direct way to attach a legacy tag to a stack anymore. Instead this rebuilds the old
	// pre-componentization {id, Count, tag} shape and runs it through the exact DataFixerUpper pipeline
	// vanilla itself uses to upgrade old worlds/structures/schematics - ItemStackComponentizationFix
	// (registered against TypeReferences.ITEM_STACK) converts the legacy tag into real components, so
	// every preset above works unmodified instead of being hand-rewritten component by component.
	private static final int LEGACY_DATA_VERSION = 3337; // 1.19.4

	private ItemStack legacyNbtToItemStack(NbtCompound legacyStackNbt) {
		Dynamic<NbtElement> dynamic = new Dynamic<>(NbtOps.INSTANCE, legacyStackNbt);
		NbtElement fixed = Schemas.getFixer()
				.update(TypeReferences.ITEM_STACK, dynamic, LEGACY_DATA_VERSION, SharedConstants.getGameVersion().dataVersion().id())
				.getValue();

		RegistryOps<NbtElement> registryOps = RegistryOps.of(NbtOps.INSTANCE, mc.world.getRegistryManager());
		return ItemStack.CODEC.parse(registryOps, fixed).result().orElse(ItemStack.EMPTY);
	}
